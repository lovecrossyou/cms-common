  ...
  if (!P.common.name_reg.test(data.user_name)) {
    ...
    if (rows.length > 0) {
      ...
      if (rows.length > 0) {
        ...
        const result = await connection.execute('INSERT INTO `user` (user_name,pass_word,create_time,login_ip,user_email) VALUES (?,?,?,?,?)', [data.user_name, data.pass_word, new Date(), P.getClientIP(ctx), data.user_email])
